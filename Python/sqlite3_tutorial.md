...
<br>
If'd we are interested to calulate the hours between two `DATETIME()` timestamps, we can could use the handy `SRFTIME()` function like this <br><br>
...
<br>And to calculate the difference in hours between the current `DATETIME` and a given `DATETIME` string, we could use the following SQLite syntax:  
